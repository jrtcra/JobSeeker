...
const Home = ({ username }) => {
    ...
    const addApplication = async () => {
        if (
            ...
            try {
                ...
                const res = await axios.post(`https://job-seeker-backend.vercel.app/api/application`, {username:username, name:name, date:date, company:company, status:status, webpage:webpage, notes:notes})
                if (res.status === 201) {
    ...
    const deleteApplication = async (index)  => {
        ...
        const res = await axios.delete(`https://job-seeker-backend.vercel.app/api/application`, {
